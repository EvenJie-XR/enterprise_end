<template>
    <el-dialog align-center id="edit-food-dialog-container" title="修改菜品" @open="onOpen" @close="onClose">
        <el-scrollbar max-height="60vh">
            <el-form label-position="left" label-width="118px" ref="formRef" :model="form" :rules="rules">
                ...
                <el-form-item label="口味描述:" prop="tasteList">
                    <div class="taste-container">
                        ...
                        <el-scrollbar class="taste-list-container" max-height="240px">
                            <!-- 口味item -->
                            <div class="taste-item-container" v-for="(tasteItem, index) in form.flavors" :key="index">
                                <!-- 口味名称 -->
                                <div>
                                    <span>口味名称:</span>
                                    <el-input v-model="tasteItem.name" placeholder="请输入口味名称..." class="input" />
                                </div>
                                <!-- 口味值列表 -->
                                <div class="taste-value-list-container">
                                    <div class="taste-value-list-header-container">
                                        <span>口味值列表:</span>
                                        <el-button color="#28A767" @click="onAddTasteValueBtnClick(tasteItem)"
                                            size="small">新增口味值</el-button>
                                    </div>
                                    <div class="taste-value-list-item-container"
                                        v-for="(_value, index) in tasteItem.value" :key="index">
                                        <!-- 口味值 -->
                                        <el-input v-model="tasteItem.value[index]" placeholder="请输入口味值..."
                                            class="input" />
                                        <el-button type="danger" size="small" class="btn"
                                            @click="onDeleteTasteValueOfTasteItem(tasteItem.value, index)">删除</el-button>
                                    </div>
                                </div>
                                <hr />
                                <el-button type="danger"
                                    @click="onDeleteTasteItemOfTasteListByTasteItemIndex(form.flavors, index)"
                                    style="width: 100%;">删除当前口味项</el-button>
                            </div>
                        </el-scrollbar>
                    </div>
                </el-form-item>
                <el-form-item label="菜品规格:" prop="specifications">
                    <div class="specification-container">
                        ...
                        <el-scrollbar class="specification-content-container" max-height="240px">
                            <!-- 规格项 -->
                            <div class="specification-item-container"
                                v-for="(specificationItem, index) in form.specifications" :key="index">
                                <!-- 规格项名称 -->
                                <div class="specification-item-name-container">
                                    <span>规格项名称:</span>
                                    <el-input v-model="specificationItem.name" placeholder="请输入规格项名称..."
                                        class="input" />
                                </div>
                                <div class="specification-item-valueHeader">
                                    <span>规格项值列表:</span>
                                    <el-button color="#28A767" size="small"
                                        @click="onAddSpecificationValueInSpecificationItemBtnClick(specificationItem.value)">新增规格值</el-button>
                                </div>
                                <!-- 规格项值 -->
                                <div class="specification-item-valueList-container">
                                    <div class="valueList-item-container"
                                        v-for="(specificationValue, index) in specificationItem.value" :key="index">
                                        <!-- 规格项值-名字 -->
                                        <div class="valueList-item-name-container">
                                            <span>规格名称:</span>
                                            <el-input v-model="specificationValue.value" placeholder="请输入规格名称..."
                                                class="input" />
                                        </div>
                                        <!-- 规格项值-线下价格 -->
                                        <div class="valueList-item-price-container">
                                            <span>线下规格价格:</span>
                                            <el-input-number v-model="specificationValue.offlinePrice" placeholder="请输入线下规格价格..."
                                                class="input" />
                                        </div>
                                        <!-- 规格项值-原价 -->
                                        <div class="valueList-item-price-container">
                                            <span>规格原价:</span>
                                            <el-input-number v-model="specificationValue.originalPrice" placeholder="请输入规格原价..."
                                                class="input" />
                                        </div>
                                        <!-- 规格项值-价格 -->
                                        <div class="valueList-item-price-container">
                                            <span>规格价格:</span>
                                            <el-input-number v-model="specificationValue.price" placeholder="请输入规格价格..."
                                                class="input" />
                                        </div>
                                        <hr />
                                        <el-button type="danger" size="small"
                                            @click="onDeleteSpecificationValueBtnClick(specificationItem.value, index)"
                                            style="width: 100%;">删除</el-button>
                                    </div>
                                </div>
                                <hr />
                                <el-button type="danger"
                                    @click="onDeleteSpecificationBtnClick(form.specifications, index)"
                                    style="width: 100%;">删除</el-button>
                            </div>
                        </el-scrollbar>
                    </div>
...
import { editFood, getAllCategory, uploadFile } from '../../api/FoodManager';
...
const food = defineModel<any>('food');
const form = ref<{
    category: number,
    description: string,
    name: string,
    ...
    category: food?.value?.categoryId,
    description: food?.value?.description,
    name: food?.value?.name,
    offlinePrice: food?.value?.offlinePrice,
    originalPrice: food?.value?.originalPrice,
    price: food?.value?.price,
    status: food?.value?.status,
...
const rules = reactive<FormRules<typeof form>>({
    ...
    specifications: [{ required: true, trigger: 'blur', message: "图片不能为空" }],
})
// 分类选项list
...
const onSaveBtnClick = () => {
    ...
    formRef.value.validate((valid) => {
        if (valid) {
            const specifications = form.value.specifications.map(specification => {
                return {
                    name: specification.name,
                    value: JSON.stringify(specification.value.map(value => {
                        return {
                            value: value.value,
            ...
            const flavors = form.value.flavors.map(taste => {
                return {
                    name: taste.name,
                    value: JSON.stringify(taste.value)
                }
            })
            editFood({
...
const onOpen = () => {
    form.value = {
        ...
        status: food?.value?.status,
        image: food?.value?.image,
        flavors: food.value.flavors.map((flavor: {
            name: string,
            value: string
        }) => {
            return {
        ...
        specifications: food.value.specifications.map((specification: {
            name: string,
            value: string
        }) => {
            return {
                name: specification.name,
                value: JSON.parse(specification.value)
            }
        })
    }
    console.log(form.value);
